...
import { scaleBand, scaleLinear, scaleLog, scaleOrdinal, scaleSqrt } from 'd3-scale';
...
const PLANETS = planets.map((d) => ({
	// type: d.pl_radius ? getPlanetType(d.pl_radius) : "Radius Unknown",
	...d
}));
...
export class Scatterplot extends React.Component {
	...
	getLinearScale = (domain, range) => {
		const ls = scaleLinear().domain(domain).range(range).nice();
		return ls;
	};
	getLogScale = (domain, range) => {
		const ls = scaleLog().domain(domain).range(range).nice();
		return ls;
	};
	getBandScale = (domain, range) => {
		const bs = scaleBand().domain(domain).range(range);
		return bs;
	};
	getColorScale = (domain, range) => {
		const cs = scaleOrdinal().domain(domain).range(range);
		return cs;
	};
	getRadiusScale = (domain, range) => {
		const rs = scaleSqrt().domain(domain).range(range);
		return rs;
	};
	...
	componentDidMount() {
		...
		//   CHART_WIDTH
		// ]);
		// const radiiExtent = extent(planets, d => d.pl_radius);
		...
		const starTemperatureExtent = extent(PLANETS, (d) => d.st_Teff);
		const starTemperatureScale = this.getLinearScale(TEMP_EXTENT, [ CHART_HEIGHT, 0 ]);
	...
	render() {
		...
		const planetTypeColors = this.getColorScale(PLANET_TYPES.map((d) => d.label), schemeDark2);
		...
		const planetRadiusScale = this.getLinearScale(extent(PLANETS, (d) => d.pl_radius), [ 0, 40 ]);
		...
		return (
			...
			>
				<g transform={`translate(${MARGIN.LEFT}, ${CHART_HEIGHT + MARGIN.TOP})`} ref={this.axisX} />
				<text className="scatterplot-x-axis-label" id="scatterplot-x-axis-label" x={W} y={H}>
					→ planet / star distance (AU)
				</text>
				<g transform={`translate(${MARGIN.LEFT}, ${MARGIN.TOP})`} ref={this.axisY} />
				<g transform={`translate(${MARGIN.LEFT}, ${MARGIN.TOP})`} className="scatterplot-planet-type-group">
					{PLANET_TYPES.map((d) => {
						return (
							<React.Fragment>
								{/* <rect
          ...
          >
            {"Habitable Zone"}
				...
				<g transform={`translate(${MARGIN.LEFT}, ${MARGIN.TOP})`}>
					<g className="scatterplot-circle-group">
						{PLANETS.filter((d) => d.pl_distance < 2).map((d, i) => (
							<React.Fragment>
								{d.pl_radius &&
								d.pl_distance &&
								d.st_Teff && (
									<circle
										...
										cx={planetStarDistScale(d.pl_distance)}
										cy={starTemperatureScale(d.st_Teff)}
										r={planetRadiusScale(d.pl_radius)}
										fill={d.pl_habitable === 1 ? 'crimson' : '#DDDDDD'}
										fillOpacity={d.pl_habitable === 1 ? 1 : 0.1}
										stroke={d.pl_habitable === 1 ? 'crimson' : '#DDDDDD'}
										strokeOpacity={d.pl_habitable === 1 ? 1 : 0.4}
										className="scatterplot-dot"
										onMouseEnter={() =>
											console.log(
												`radus: ${d.pl_radius}, orbital period: ${d.pl_orbper}, discovery method: ${d.pl_discmethod},
            ...
            {PLANETS.filter(d => d.pl_distance < 2).map((d, i) => (
              <React.Fragment>
                {d.pl_habitable === 1 && (
                  <text
                    x={planetStarDistScale(d.pl_distance)}
                    y={starTemperatureScale(d.st_Teff)}
							...
							<React.Fragment>
								{d.pl_habitable === 1 && (
									<line
										...
										x1={planetStarDistScale(d.st_inner)}
										y1={starTemperatureScale(d.st_Teff)}
										x2={planetStarDistScale(d.st_outer)}
										y2={starTemperatureScale(d.st_Teff)}
										stroke={d.pl_habitable === 1 ? 'crimson' : '#DDDDDD'}
										strokeWidth={0.5}
									/>
								)}
